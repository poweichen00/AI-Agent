...
# 功能：驗證完整的端到端 agent 鏈路：呼叫真實 LLM → 執行 read_file → 成功完成並寫入 events.jsonl
# 設計：使用真實 ANTHROPIC_API_KEY 和真實檔案，goal 中指定一個具體的數字（7391）以便斷言 LLM 確實讀了檔案；
#       透過 events.jsonl 的事件序列斷言每個關鍵階段都被記錄，而非只檢查 stdout，因為 events.jsonl 是 S1 的核心驗收產物
