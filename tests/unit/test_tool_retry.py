...
# 功能：驗證 runtime_error 在首次失敗後自動重試，最多 2 次，第 2 次成功時返回 ok
# 設計：_FailNTimes(1) 第一次返回 runtime_error，第二次成功；monkeypatch 消除 sleep 延遲
...
# 功能：驗證 rate_limited 異常觸發重試，重試成功後返回正常結果
# 設計：_RateLimitedNTimes(1) 拋 RateLimitedError 一次，第二次成功；檢查 error_class 欄位
...
# 功能：驗證 runtime_error 超過 2 次重試後最終返回失敗，attempt 欄位遞增
# 設計：_AlwaysFails 三次都失敗；斷言最終結果 is_error + 收到 3 個 failed 事件，attempt 為 1/2/3
...
# 功能：驗證 rate_limited 耗盡重試後最終返回失敗，error_class 為 rate_limited
# 設計：_RateLimitedNTimes(10) 始終拋異常，斷言 3 個 failed 事件且 error_class 統一
...
# 功能：驗證 schema_error 不觸發重試，直接失敗
# 設計：_AlwaysFails("schema_error") 首次即 schema 錯誤，斷言只發一次 failed 事件且 attempt=1
...
# 功能：驗證 timeout_error 不觸發重試，直接失敗
# 設計：SlowTool 配合極短超時觸發 TimeoutError；斷言只發一次 failed 事件，不重試（重試會再次超時）
