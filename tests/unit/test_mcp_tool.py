...
from agentx.core.mcp.client import McpClient, McpServerUnavailableError, McpToolDef
...
# 功能：McpTool.invoke 應呼叫 client.call_tool 並將返回值封裝為 ToolResult
# 設計：mock client.call_tool 返回固定字串，驗證 ToolResult.content 一致
@pytest.mark.asyncio
...
# 功能：工具名應以 {server_name}__ 為字首防止命名衝突
# 設計：驗證 McpTool.name 格式為 "filesystem__read_file"
def test_tool_name_prefixed() -> None:
    tool, _ = _make_tool("read_file", "filesystem")
    assert tool.name == "filesystem__read_file"


# 功能：client 拋 McpServerUnavailableError 時應返回 is_error=True 且不重新丟擲
# 設計：mock client.call_tool 拋該異常，斷言 ToolResult.is_error=True 且訊息含 server 名稱
@pytest.mark.asyncio
...
# 功能：client 拋其他異常時應返回 runtime_error 型別的 ToolResult
# 設計：mock client.call_tool 拋 RuntimeError，斷言 ToolResult 被正確包裝
@pytest.mark.asyncio
...
# 功能：input_schema 應直接使用 MCP tool_def 中的 schema，而非 pydantic model
# 設計：驗證 params_model 為 None，input_schema 與 tool_def.input_schema 一致
