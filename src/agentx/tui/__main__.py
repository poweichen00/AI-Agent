...
from agentx.tui.app import AgentXTuiApp

_DEFAULT_TUI_LOG = "~/.agentx/logs/tui.log"


# TUI 檔案日誌初始化：不寫 stderr（避免幹擾 Textual 渲染），只寫滾動檔案
def _setup_logging(level: str) -> None:
    log_path = Path(os.environ.get("AGENTX_TUI_LOG_FILE", _DEFAULT_TUI_LOG)).expanduser()
...
def main() -> None:
    ...
    app = AgentXTuiApp(config.host, config.port, replay_run_id=args.replay)
    app.run()
