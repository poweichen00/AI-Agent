...
from agentx.core.bus.events import LlmModelSelectedEvent, LlmTokenEvent, LlmUsageEvent
from agentx.core.events.bus import EventBus
from agentx.core.llm.types import LlmResponse, ToolCallBlock, UsageStats
...
# 返回指定模型的最大 context window token 數
...
class AnthropicProvider:
    # 初始化 Anthropic 客戶端；client 可在測試時注入以跳過 API key 檢查
    ...
    # 流式呼叫 Anthropic API，逐 token 釋出事件並返回 LlmResponse；網路中斷時自動重試
    async def chat(
        self,
