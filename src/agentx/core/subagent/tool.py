...
from agentx.core.agents.loader import AgentProfile, AgentProfileLoader
from agentx.core.bus.events import SubagentFinishedEvent, SubagentStartedEvent
...
from agentx.core.runs import new_run_id
...
from agentx.core.tools.builtin.task_create import TaskCreateTool
from agentx.core.tools.builtin.task_get import TaskGetTool
from agentx.core.tools.builtin.task_list import TaskListTool
from agentx.core.tools.builtin.task_update import TaskUpdateTool
...
# 在隔離的冷啟動上下文中派生子 agent，支援前臺阻塞和後臺並行兩種模式
class SpawnAgentTool(BaseTool):
    ...
    # 構造 SpawnAgentTool；depth=0 表示根 agent，最大允許巢狀深度為 2
    def __init__(
        self,
    ...
    # 派生子 agent，前臺時阻塞直到完成並返回結果，後臺時立即返回 run_id
    async def invoke(self, params: dict[str, object]) -> ToolResult:
        ...
        # 將子 bus 所有事件橋接到父 bus，TUI 據此渲染巢狀進度
    ...
    # 構造子 registry；基於角色配置過濾工具，深度允許時註冊巢狀 SpawnAgentTool
