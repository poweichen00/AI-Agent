...
from agentx.core.task.model import Task, TaskStatus
...
class TaskManager:
    # 初始化：確保目錄存在，掃描現有檔案確定下一個 ID
    def __init__(self, tasks_dir: Path) -> None:
        self._dir = tasks_dir
        self._dir.mkdir(parents=True, exist_ok=True)
        self._next_id = self._max_id() + 1

    # 掃描目錄中 task_*.json 檔案，返回最大 ID（無檔案則返回 0）
    ...
    # 更新任務狀態或依賴列表；status="completed" 時自動清理其他任務的 blocked_by
    def update(
        self,
        task_id: int,
        *,
    ...
    # 將 completed_id 從所有其他任務的 blocked_by 列表中移除
    ...
    # 格式化任務列表摘要，供 task_list 工具返回給 Agent
