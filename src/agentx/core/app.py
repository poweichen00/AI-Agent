...
import agentx
from agentx.core.bus.commands import (
    AgentRunCommand,
    AgentRunResult,
...
from agentx.core.bus.envelope import EventPushEnvelope
from agentx.core.config import AgentXConfig, get_config
from agentx.core.events.bus import EventBus
from agentx.core.llm.provider import AnthropicProvider
from agentx.core.logging_setup import setup_logging
from agentx.core.mcp.server import McpServerManager
...
from agentx.core.runs import events_file, new_run_id
from agentx.core.session import SessionManager, SessionStore
...
from agentx.core.transport.socket_server import SocketServer, get_connection_writer
...
class CoreApp:
    def __init__(self) -> None:
        ...
        self._config: AgentXConfig | None = None
        self._running_runs: set[asyncio.Task[Any]] = set()
        self._sessions: SessionManager | None = None
        self._permission_manager: PermissionManager | None = None
        self._mcp_manager: McpServerManager | None = None

    # 處理 core.ping 請求，返回服務版本、執行時長和接收時間
    async def _ping_handler(self, params: dict[str, Any]) -> PongResult:
        client = params.get("client", "unknown")
        logger.debug("ping from %s", client)
        return PongResult(
            server_version=agentx.__version__,
            uptime_ms=int((time.monotonic() - self._start_time) * 1000),
            received_at=datetime.datetime.now(datetime.UTC).isoformat(),
        )

    # 將 EventBus 事件寫入 trace（作為 EventBus 訂閱者）
    ...
    # 啟動一次 agent run：非同步建立 AgentRunner 並立即返回 run_id
    ...
    # 建立 chat 或 one_shot session，並返回 session_id
    ...
    # 向 session 傳送一條使用者訊息並同步等待對應 run 完成
    ...
    # 返回 session 的完整 Anthropic messages 歷史
    ...
    # 接收客戶端許可權審批響應，resolve 對應掛起的 Future
    ...
    # 手動壓縮 session thread，將摘要持久化寫入 thread.jsonl
    ...
    # 註冊客戶端事件訂閱，可選先回放 events.jsonl 歷史再接收即時流
    ...
    # 從 events.jsonl 向 writer 回放匹配 topic 的歷史事件，返回已回放條數
    ...
    # 啟動守護程式：載入配置、初始化日誌、啟動 trace、啟動 TCP 伺服器，並等待退出訊號
    async def run(self) -> None:
        ...
        policy_file = Path("~/.agentx/policy.toml").expanduser()
        ...
        sessions_root = Path("~/.agentx/sessions").expanduser()
        ...
        logger.info("agentx-core %s listening addr=%s", agentx.__version__, addr)
