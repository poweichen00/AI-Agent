...
from agentx.core.bus.envelope import JsonRpcRequest
...
class SocketClient:
    ...
    # 註冊伺服器推送事件的回撥，可多次呼叫以新增多個 handler
    def on_event(self, handler: EventHandler) -> None:
        self._event_handlers.append(handler)

    # 傳送 JSON-RPC 命令並等待響應，成功返回 result dict，失敗丟擲 IpcError
    ...
    # 持續讀取伺服器訊息，分發 RPC 響應到 pending future 或事件到 event handler
    ...
    # 解析單行訊息並路由到 pending future（RPC 響應）或 event handler（伺服器推送）
