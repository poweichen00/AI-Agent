...
from agentx.core.events.bus import EventBus, EventHandler
...
from agentx.core.memory.loader import load_context_file
from agentx.core.permissions.manager import PermissionManager
from agentx.core.runs import RUNS_DIR, new_run_id
...
from agentx.core.tools.builtin import (
    BashTool,
    ListDirTool,
    NoteSaveTool,
    ReadFileTool,
...
class AgentRunner:
    ...
    def __init__(
        ...
        *,
        ...
        # 跨 run 共享的後臺 subagent 任務登入檔
        self._task_registry = BackgroundTaskRegistry()

    # 構建工具登入檔，注入 TaskManager（任務工具共享同一例項）；可選注入 SpawnAgentTool
    def _build_registry(
        self,
    ...
    async def run_and_capture(
        ...
        *,
        ...
        global_ctx = load_context_file(Path("~/.agentx/context.md").expanduser())
        project_ctx = load_context_file(Path(".agentx/context.md"))
