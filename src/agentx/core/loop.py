...
from agentx.core.bus.events import StepFinishedEvent, StepStartedEvent
...
class AgentLoop:
    ...
    # 驅動 plan→act→observe 迴圈直到上下文終止；CancelledError 向上傳播
    async def run(self, context: ExecutionContext) -> None:
        while not context.is_done():
            ...
            # 工具結果追加完畢（messages 末尾為 user）後檢查壓縮，僅在 run 繼續時觸發
            # 此時壓縮結果 [user_summary, assistant_ack] 對下一次 LLM 呼叫是合法輸入
            if (
                not context.is_done()
