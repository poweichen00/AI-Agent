...
_DEFAULT_LOG_FILE = "~/.agentx/logs/core.log"
_DEFAULT_LOG_FORMAT = "text"
_DEFAULT_CONFIG_PATH = "~/.agentx/config.toml"
_DEFAULT_MAX_STEPS = 20
_DEFAULT_MODEL = "claude-sonnet-4-6"
_DEFAULT_TRACE_FILE = "~/.agentx/traces/daemon.jsonl"
...
class TraceConfig:
    ...
    include_llm_payload: bool = True  # false 時 LLM 記錄只保留摘要


@dataclass
class PermissionConfig:
    timeout_s: float = 60.0  # 審批超時秒數；0 表示不超時


@dataclass
class CompactionConfig:
    auto_threshold: float = 0.0    # context_pct 觸發自動壓縮的閾值（0 表示停用，推薦用手動 /compact）
    tool_result_limit: int = 8_000  # tool_result 截斷觸發字元數
    tool_result_keep: int = 4_000   # 截斷後保留的字首字元數
...
class McpServerConfig:
    ...
    command: str = ""              # stdio 專用：可執行檔案路徑
    args: list[str] = field(default_factory=list)
    env: dict[str, str] = field(default_factory=dict)
    host: str = "localhost"        # tcp 專用
    port: int = 3000               # tcp 專用
...
# 構建並返回執行時配置：預設值 → 全域性 TOML → 專案本地 TOML → .env → 系統環境變數（後者優先順序最高）
def get_config() -> AgentXConfig:
    config = AgentXConfig()

    # .env 必須在讀取 AGENTX_CONFIG 之前載入，以便 .env 中的 AGENTX_CONFIG 能影響 TOML 路徑
    load_dotenv(".env", override=False)

    # 若顯式指定 AGENTX_CONFIG，只讀該檔案；否則按優先順序疊加：全域性 → 專案本地
    explicit = os.environ.get("AGENTX_CONFIG")
    if explicit:
...
# 將已解析的 TOML 根表寫入 config；未知小節或型別錯誤時退出程式
def _apply_toml(config: AgentXConfig, data: dict[str, Any]) -> None:
...
# 用 AGENTX_* 環境變數覆蓋 config 中對應欄位（若變數已設定）
def _apply_env(config: AgentXConfig) -> None:
    host = os.environ.get("AGENTX_HOST")
    if host is not None:
        config.host = host

    port_str = os.environ.get("AGENTX_PORT")
    if port_str is not None:
        try:
            config.port = int(port_str)
        except ValueError:
            raise SystemExit(f"Config error: AGENTX_PORT must be an integer, got: {port_str!r}")

    log_level = os.environ.get("AGENTX_LOG_LEVEL")
    if log_level is not None:
        config.logging.level = log_level

    log_file = os.environ.get("AGENTX_LOG_FILE")
    if log_file is not None:
        config.logging.file = log_file

    log_format = os.environ.get("AGENTX_LOG_FORMAT")
    if log_format is not None:
        config.logging.format = log_format

    max_steps_str = os.environ.get("AGENTX_MAX_STEPS")
    if max_steps_str is not None:
        try:
            val = int(max_steps_str)
            if val <= 0:
                raise SystemExit(
                    "Config error: AGENTX_MAX_STEPS must be a positive integer,"
                    f" got: {max_steps_str!r}"
                )
            config.agent.max_steps = val
        except ValueError:
            raise SystemExit(
                f"Config error: AGENTX_MAX_STEPS must be an integer, got: {max_steps_str!r}"
            )

    default_model = os.environ.get("AGENTX_LLM_DEFAULT_MODEL")
    if default_model is not None:
        config.llm.default_model = default_model

    trace_enabled = os.environ.get("AGENTX_TRACE_ENABLED")
    if trace_enabled is not None:
        config.trace.enabled = trace_enabled.lower() not in ("0", "false", "no")

    trace_file = os.environ.get("AGENTX_TRACE_FILE")
    if trace_file is not None:
        config.trace.file = trace_file

    trace_payload = os.environ.get("AGENTX_TRACE_INCLUDE_LLM_PAYLOAD")
    if trace_payload is not None:
        config.trace.include_llm_payload = trace_payload.lower() not in ("0", "false", "no")

    perm_timeout = os.environ.get("AGENTX_PERMISSION_TIMEOUT_S")
    if perm_timeout is not None:
        try:
            perm_timeout_val = float(perm_timeout)
            if perm_timeout_val < 0:
                raise SystemExit(
                    f"Config error: AGENTX_PERMISSION_TIMEOUT_S must be >= 0, got: {perm_timeout!r}"
                )
            config.permission.timeout_s = perm_timeout_val
        except ValueError:
            raise SystemExit(
                f"Config error: AGENTX_PERMISSION_TIMEOUT_S must be a number, got: {perm_timeout!r}"
            )

    compact_threshold = os.environ.get("AGENTX_COMPACT_THRESHOLD")
    if compact_threshold is not None:
        try:
            compact_threshold_val = float(compact_threshold)
            if not (0.0 <= compact_threshold_val <= 1.0):
                raise SystemExit(
                    f"Config error: AGENTX_COMPACT_THRESHOLD must be between 0 and 1, got: {compact_threshold!r}"
                )
            config.compaction.auto_threshold = compact_threshold_val
        except ValueError:
            raise SystemExit(
                f"Config error: AGENTX_COMPACT_THRESHOLD must be a number, got: {compact_threshold!r}"
            )

    compact_tool_limit = os.environ.get("AGENTX_COMPACT_TOOL_LIMIT")
    if compact_tool_limit is not None:
        try:
            compact_tool_limit_val = int(compact_tool_limit)
            if compact_tool_limit_val <= 0:
                raise SystemExit(
                    f"Config error: AGENTX_COMPACT_TOOL_LIMIT must be a positive integer, got: {compact_tool_limit!r}"
                )
            config.compaction.tool_result_limit = compact_tool_limit_val
        except ValueError:
            raise SystemExit(
                f"Config error: AGENTX_COMPACT_TOOL_LIMIT must be an integer, got: {compact_tool_limit!r}"
            )

    compact_tool_keep = os.environ.get("AGENTX_COMPACT_TOOL_KEEP")
    if compact_tool_keep is not None:
        try:
            compact_tool_keep_val = int(compact_tool_keep)
            if compact_tool_keep_val <= 0:
                raise SystemExit(
                    f"Config error: AGENTX_COMPACT_TOOL_KEEP must be a positive integer, got: {compact_tool_keep!r}"
                )
            config.compaction.tool_result_keep = compact_tool_keep_val
        except ValueError:
            raise SystemExit(
                f"Config error: AGENTX_COMPACT_TOOL_KEEP must be an integer, got: {compact_tool_keep!r}"
            )
