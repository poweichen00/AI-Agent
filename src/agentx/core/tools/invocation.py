...
from agentx.core.bus.events import (
...
from agentx.core.tools.base import ToolResult
...
# 釋出 ToolCallFailedEvent 並返回對應 ToolResult
async def _fail(
    bus: EventBus,
    run_id: str,
...
# 校驗引數、檢查許可權、限時呼叫工具、釋出進度事件，失敗時指數退避重試，返回 ToolResult（不拋異常）
