...
class StdoutPrinter:
    # 接收 dict 格式的事件並將執行進度格式化列印到終端
    def __init__(self) -> None:
        self._inline = False  # True while LLM tokens are mid-line
        self._run_start: float = 0.0

    # 若當前行有未換行的 token，補一個換行符
    def _ensure_newline(self) -> None:
        if self._inline:
            print()
            self._inline = False

    # 根據事件 type 欄位分發並格式化列印到 stdout/stderr
...
# 非同步核心：連線 daemon，訂閱事件，觸發 run，等待 run.finished
async def _run_async(goal: str, config: AgentXConfig) -> int:
...
# 執行 agentx run --goal "..." 命令
def cmd_run(goal: str, config: AgentXConfig) -> None:
    try:
