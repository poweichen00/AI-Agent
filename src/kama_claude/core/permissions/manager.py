...
# 管理工具呼叫許可權：策略評估、使用者審批掛起、session 級和持久化 always 快取、超時
class PermissionManager:
    def __init__(
        ...
        *,
        ...
        # (session_id, tool_name) → "allow" | "deny"（session 記憶體，重啟丟失）
        self._session_always: dict[tuple[str, str], str] = {}
        # tool_name → "allow" | "deny"（持久化，從 policy_file 載入）
    ...
    # 檢查許可權；如需 ask 則向客戶端發事件並等待響應；返回 (allowed, decision_str)
    async def check_and_wait(
        ...
        # Tier 1: deny_patterns（bash only，不可被快取繞過）
        ...
        # Tier 2: OUTSIDE_CWD_HEURISTICS（bash only，強制 ASK，不可被任何快取繞過）
        outside_cwd = bool(command and matches_outside_cwd(command))

        if not outside_cwd:
            # Tier 3: session always 快取
        ...
        # ASK 路徑（來自 OUTSIDE_CWD 強制 ASK，或 default=ASK）
    ...
    # 處理客戶端返回的審批決策，resolve 對應 Future
    ...
    # 應用審批決策，更新 session + persistent 快取，返回是否放行
